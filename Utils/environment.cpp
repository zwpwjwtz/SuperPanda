...
{
    auto f = std::mem_fn(&EnvironmentItem::name);
    using const_ref = typename QList<EnvironmentItem>::const_reference;
...
QStringList EnvironmentItem::toStringList(const QList<EnvironmentItem> &list)
{
    ...
    for (int i=0; i<list.count(); i++)
        tempList.push_back(list[i].name + '=' + list[i].value);
    return tempList;
}

QList<EnvironmentItem> EnvironmentItem::itemsFromVariantList(const QVariantList &list)
{
    QList<EnvironmentItem> tempList;
    for (int i=0; i<list.count(); i++)
        tempList.append(itemFromVariantList(list[i].toList()));
    return tempList;
}
...
QVariantList EnvironmentItem::toVariantList(const QList<EnvironmentItem> &list)
{
    ...
    for (int i=0; i<list.count(); i++)
        tempList.push_back(QVariant(toVariantList(list[i])));
    return tempList;
}
...
void Environment::prependOrSetLibrarySearchPaths(const QStringList &values)
{
    for (int i=values.count() - 1; i>=0; i--)
        prependOrSetLibrarySearchPath(values[i]);
}
...
FileNameList Environment::path() const
{
    ...
    for (int i=0; i<pathComponents.count(); i++)
        tempList.push_back(FileName::fromUserInput(pathComponents[i]));
    return tempList;
}
...
QStringList Environment::expandVariables(const QStringList &variables) const
{
    ...
    for (int i=0; i<variables.count(); i++)
        tempList.push_back(expandVariables(variables[i]));
    return tempList;
}
